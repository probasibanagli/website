...
file_path = r"c:\Users\lokes\Downloads\website\src\app\admin\emergency\page.tsx"

with open(file_path, "r", encoding="utf-8") as f:
    content = f.read()

# Replace openAdd to handleDelete
old_methods = r"  function openAdd\(\) \{.*?(?=  if \(!canView\))"
new_methods = """  function toggleLanguage(lang: string) {
  ...
  async function handleSave() {
    ...
    try {
      ...
      if (editId) {
        from firebase.firestore import updateDoc, doc
...
content = content.replace("from firebase.firestore import updateDoc, doc\n        ", "")
...
# Table / List replacement
old_table = r"<table className=\"w-full\">.*?(?=</table >|</table>)\s*</table>"
new_table = """{activeTab === 'doctors' ? (
...
old_form = r"            <div className=\"flex-1 p-6 overflow-y-auto space-y-5\">.*?</div>\s*<div className=\"p-6 border-t border-border flex justify-end gap-3 bg-surface/30\">"

new_form = """            <div className="flex-1 p-6 overflow-y-auto space-y-5">
              {activeTab === 'hospitals' ? (
                <>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
...
content = content.replace("Add Doctor", "Add Staff / Doctor")
